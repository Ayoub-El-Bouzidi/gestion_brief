...
if (isset($_POST['login'])) {
    ...
    foreach($result as $row){
        if($email==$row->EMAIL && $password==$row->MOT_DE_PASSE){
         
            $_SESSION['id'] = $row->ID_FORMATEUR;
            header('Location: formateur.php');
        }
    }
    $query2='SELECT * FROM apprenant';
    $stmtapr=$conn->prepare($query2);
    $stmtapr->execute();
    $res=$stmtapr->fetchAll(PDO::FETCH_OBJ);

    foreach($res as $app){
        if($app->EMAIL==$email && $app->$password){
